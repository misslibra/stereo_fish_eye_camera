...
void load_image_points(int board_width, int board_height, float square_size, int num_imgs, 
  ...
  for (int i = 1; i <= num_imgs; i++) {
    ...
    char left_img[300], right_img[300];
...
int main(int argc, char const *argv[])
{
  ...
  std::cout<<object_points.size()<< " "<<left_img_points.size()<<" "<<right_img_points.size() <<std::endl;
...
/////////////// undistortimage
  ...
  for (int i = 1; i <= num_imgs; i++) {
    ...
    sprintf(left_img, "%s%s%d.jpg", img_dir, leftimg_filename, i);
    img1 = imread(left_img, CV_LOAD_IMAGE_COLOR);
    // cv::resize(img1,img1,cv::Size(),0.5,0.5,INTER_CUBIC );
    if (!img1.data)
    {
      std::cout<<"read image fail when distore "<<std::endl;
      continue;
    }
    cv::Mat outimg;
    cv::fisheye::undistortImage(img1,outimg,K1,D1,K1);
    cv::imwrite("/cv/cindy/stereo_vision/calibration_rectify/fisheye-stereo-calibration-master/result/resize_distore_" + std::to_string(i) + ".jpg", outimg);
    cv::imshow("after rectify",outimg);
    cv::waitKey(0);
  }

  // cv::Mat imginput = cv::imread("/cv/cindy/stereo_vision/calibration_rectify/fisheye-stereo-calibration-master/DongHuaFisheyecamera/left11.jpg",CV_LOAD_IMAGE_COLOR);
  // cv::Matx33d KK1(7.2512703456484553e+02, 0., 1.3084379859812832e+03, 0., 7.2444418376279964e+02, 9.5961031123063265e+02, 0., 0., 1.);
  // cv::Vec4d DD1(-2.9814828361951921e-02, -4.7684884926221327e-03, -1.2196199537986071e-03, 1.1378548557740654e-04);
  // cv::Mat outimg;
  // cv::fisheye::undistortImage(imginput,outimg,KK1,DD1,KK1);
  // int start_width =  336;
  // int start_height = 432;
  // cv::Rect roi;
  // roi.x = start_width;
  // roi.y = start_height;
  // roi.width = outimg.size().width - (start_width*2);
  // roi.height = outimg.size().height - (start_height*2);
  // cv::Mat crop = outimg(roi);
  // cv::imwrite("/cv/cindy/stereo_vision/calibration_rectify/fisheye-stereo-calibration-master/result/left9_11.bmp", crop);
